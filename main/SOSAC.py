...
from src.SAC_model import Q_phi_separate_Network,GaussianPolicy,DeterministicPolicy
...
class SAC(object):
    def __init__(self, num_inputs, action_space):
        ...
        self.critic = Q_phi_separate_Network(num_inputs, action_space.shape[0],hidden_dim = args.hidden).to(DEFAULT_DEVICE)
        self.critic_optim = torch.optim.Adam(self.critic.parameters(), lr=args.lr)

        self.critic_target = Q_phi_separate_Network(num_inputs, action_space.shape[0]).to(DEFAULT_DEVICE)
    ...
    def update_parameters(self, memory, updates):
        ...
        if len(self.dirty_batch[0])>=100:
            with torch.no_grad():
                ...
                _, _,phi1, _,_,_ = self.critic(state_batch, action_batch)
        ...
        qf1, qf2, phi1, phi2, UCB1, UCB2 = self.critic(state_batch, action_batch)  # Two Q-functions to mitigate positive bias in the policy improvement step

        with torch.no_grad():
            double_phi = phi1.to(torch.float64)
            temp_phi_squre = torch.mm(double_phi, self.eig_vector)**2
            temp_eig_value = self.eig_value+10.0
            inverse_count = torch.sum(temp_phi_squre/temp_eig_value,1,keepdim = True)
            UCB_bonus = (inverse_count**0.5).to(torch.float32)
            next_state_action, next_state_log_pi, _ = self.policy.sample(next_state_batch)
            qf1_next_target, qf2_next_target,_,_,UCB1_target,UCB2_target = self.critic_target(next_state_batch, next_state_action)
            min_qf_next_target = torch.min(qf1_next_target, qf2_next_target) - self.alpha * next_state_log_pi
            next_q_value = reward_batch + mask_batch * self.gamma * (min_qf_next_target)
            min_UCB_next_target = torch.min(UCB1_target, UCB2_target)
            next_UCB_value = UCB_bonus + mask_batch * self.gamma * (min_UCB_next_target)
            UCB_logs = UCB_bonus.mean().cpu()
        ...
        UCB1_loss = torch.nn.functional.mse_loss(UCB1, next_UCB_value)
        UCB2_loss = torch.nn.functional.mse_loss(UCB2, next_UCB_value)
        pi, log_pi, _ = self.policy.sample(state_batch)

        qf1_pi, qf2_pi,_,_,UCB1,UCB2 = self.critic(state_batch, pi, True)

        min_qf_pi = torch.min(qf1_pi, qf2_pi)+args.beta*torch.min(UCB1,UCB2)

        policy_loss = ((self.alpha * log_pi) - min_qf_pi).mean() # Jπ = 𝔼st∼D,εt∼N[α * logπ(f(εt;st)|st) − Q(st,f(εt;st))]

        self.critic_optim.zero_grad()
        (qf1_loss+UCB1_loss).backward()
        self.critic_optim.step()

        self.critic_optim.zero_grad()
        (qf2_loss+UCB2_loss).backward()
        self.critic_optim.step()
