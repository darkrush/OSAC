    ...
    def update_parameters(self, memory, updates):
        ...
        if len(self.dirty_batch[0])>0:
            with torch.no_grad():
                ...
                self.matrix=self.matrix+torch.mm(phi1.transpose(0,1),phi1)
                #self.feature_number += new_number
                self.inv_matrix = torch.cholesky_inverse(torch.cholesky(self.matrix+torch.eye(256,256).to(DEFAULT_DEVICE)))
